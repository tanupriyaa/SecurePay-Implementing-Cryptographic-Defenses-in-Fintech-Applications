...
def isEmail(txt): return True if re.match(r'[a-zA-Z0-9.-]+@[a-zA-Z0-9.-]+\.[a-zA-Z0-9_-]+', txt) else False
def isPassword(txt,_min=8, _max=12): return True if re.match(r'^(?=.[a-z])(?=.[A-Z])(?=.\d)(?=.[@$!%?&])[A-Za-z\d@$!%?&]{' + str(_min) + ',' + str(_max)+'}$', txt) else False
...
#transaction management fucntions
...
def downloadHistory(opt='EXCEL'):
    ...
    if opt == 'EXCEL':
        download_df.to_excel('output.xlsx', index=False)
        return 1
...
def decryptDatabase():
    try:
        ...
        # Handle the error here, such as logging or prompting the user
                            ...
                            if main_chc == '1':
                                ...
                                if chc == '1':
                                    ...
                                    if ch == '0':
                                        ...
                                        secret = fetchSecret(USER_PATH, username)  # Retrieve the secret from the database
                                        if secret:
                                            editActivation(USER_PATH, username, mode='ENABLE')
                                            totp = pyotp.totp.TOTP(secret)
                                            provisioning_uri = totp.provisioning_uri(name=username, issuer_name='Crypto Project')
        
                                            # Create QR code
                                            url = pyqrcode.create(provisioning_uri)
                                            ...
                                            print("Failed to retrieve secret from the database. Activation unsuccessful.")
